...
extern void rspTrace(char const * szFrmt, ...);
...
extern short rspAssert(	// Returns result.
	char const * pszFile,			// Source file.
	short sLine,			// Source line.
	char const * pszExpr);		// String representing expression.
...
extern void rspGetMouse(
	short* psX,				// X position returned here (unless nullptr)
	short* psY,				// Y position returned here (unless nullptr)
	short* psButtons);	// button status returned here (unless nullptr)
...
extern short rspGetMouseEvent(	// Returns 0 if no event was available, non-zero otherwise
	...
	long* plTime = nullptr,					// Event's time stamp returned here (unless nullptr)
...
extern short rspGetKey(						// Returns 1 if key was available, 0 if not
	...
	long* plTime = nullptr);					// Out: Key's time stamp (unless nullptr)
...
extern void rspGetJoyPos(
	short sJoy,					// In:  Joystick to query.
	long *plX,					// Out: X axis position of joystick, if not nullptr.
	long *plY = nullptr,			// Out: Y axis position of joystick, if not nullptr.
	long *plZ = nullptr,			// Out: Z axis position of joystick, if not nullptr.
	long *plW = nullptr,			// Out: W axis position of joystick, if not nullptr.
	long *plU = nullptr,			// Out: U axis position of joystick, if not nullptr.
	long *plV = nullptr);		// Out: V axis position of joystick, if not nullptr.
...
extern void rspGetJoyState(
	short sJoy,					// In:  Joystick to query.
	U32*	pu32Buttons,		// Out: Buttons that are down, if not nullptr.
									// An RSP_JOY_BUT_## bit field that is set indicates
									// that button is down.
	U32*	pu32Axes = nullptr);	// Out: Directions that are specificed, if not nullptr.
...
extern short rspQueryVideoMode(			// Returns 0 for each valid mode, then non-zero thereafter
	short* psDeviceDepth,					// Out: Device depth (unless nullptr)
	short* psDeviceWidth = nullptr,			// Out: Device width (unless nullptr)
	short* psDeviceHeight = nullptr,			// Out: Device height (unless nullptr)
	short* psDevicePages = nullptr);			// Out: Maximum number of pages supported (unless nullptr)
...
extern short rspGetVideoMode(				// Returns 0 if sucessfull, non-zero otherwise
	...
	short* psDeviceHeight = nullptr,			// Out: Device height (unless nullptr)
	short* psDevicePages = nullptr,			// Out: Maximum number of pages supported (unless nullptr)
	short* psWidth = nullptr,					// Out: Window width or -1 (unless nullptr)
	short* psHeight = nullptr,					// Out: Window height or -1 (unless nullptr)
	short* psPages = nullptr,					// Out: Number of pages or -1 (unless nullptr)
	short* psScaling = nullptr);				// Out: Scaling flag or -1 (unless nullptr)
...
extern void rspSetPaletteEntries(
	...
	unsigned char const * pucRed,					// In:  Pointer to starting source red value
	unsigned char const * pucGreen,				// In:  Pointer to starting source green value
	unsigned char const * pucBlue,					// In:  Pointer to starting source blue value
...
extern short rspGetSoundOutMode(				// Returns 0 if successfull, non-zero otherwise
	long* plSampleRate,							// Out: Sample rate or -1 (unless nullptr)
	long* plBitsPerSample = nullptr,				// Out: Bits per sample or -1 (unless nullptr)
	long* plChannels = nullptr,					// Out: Channels (mono=1, stereo=2) or -1 (unless nullptr)
	long* plCurBufferTime = nullptr,				// Out: Current buffer time or -1 (unless nullptr)
	long* plMaxBufferTime = nullptr);			// Out: Maximum buffer time or -1 (unless nullptr)
...
extern void rspSetApplicationName(char const * pszName);
...
extern int rspCommandLine(char const *cmd);
