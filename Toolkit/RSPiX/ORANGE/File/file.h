	...
	public:	// Methods.
		...
		short SetBufferSize(size_t stBufSize)
			...
			if (IsFile() == TRUE)
				...
				return (short)setvbuf(m_fs, nullptr, _IOFBF, stBufSize);
				}
			else
				if (IsMemory() == TRUE)
					return 0;
...
//////////////////////////////////////////////////////////////////////////////
	...
	public:	// Querries.
		...
		short IsOpen(void)	{ return (m_fs == nullptr && m_pucFile == nullptr ? FALSE : TRUE); }

		// Returns TRUE, if connected to a non-memory file.
		short IsFile(void)	{ return (m_fs == nullptr ? FALSE : TRUE); }

		// Returns TRUE, if connected to a memory file.
		short IsMemory(void)	{ return (m_pucFile == nullptr ? FALSE : TRUE); }
