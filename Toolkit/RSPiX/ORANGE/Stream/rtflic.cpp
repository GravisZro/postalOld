...
int16_t CRtFlic::Use(	uint8_t* puc, int32_t lSize, uint16_t usType, uint8_t ucFlags, 
	...
	if (ucFlags & RT_FLAG_INIT)
		...
		if (pflxhdr->pImage)
			{
			ASSERT(pflxhdr->pImage->pData);
			ASSERT(pflxhdr->pImage->pPalette);
			ASSERT(pflxhdr->pImage->pPalette->pData);
