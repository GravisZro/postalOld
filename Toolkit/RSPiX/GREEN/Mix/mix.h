...
class RMix
	...
	public:	// Enums and typedefs.
		enum Msg {	// Mix messages (callback can receive in addition to blue messages).
			Suspended,	// Channel has finished.
			Data			// Channel needs new data.
		};

		enum State {	// Mix states for ms_sState and GetState().
	...
	public:	// Querries.
		...
		static short GetMode(						// Returns 0 on success; 
			...
			long*		plDevBitsPerSample = nullptr,	// Bits per sample of device,
															// returned here, if not nullptr.
			long*		plNumChannels = nullptr,		// Number of channels (1 == mono, 
															// 2 == stereo) returned here, 
															// if not nullptr.
			long*		plBufferTime = nullptr,			// Amount of time in ms to lead the 
															...
															// calls to rspDoSound.
			long*		plMaxBufferTime	= nullptr,	// Maximum buffer time.  This is the amt
			...
			long*		plMixBitsPerSample = nullptr,	// Bits per sample at which samples are
															// mixed, if not nullptr.
			long*		plSrcBitsPerSample = nullptr);// Bits per sample at which samples must
