...
inline bool NextItem(	// Returns true, if new item selected.
	...
	if (sNumItemsTraversed > ms_sNumMenuItems)
		...
		if (ms_pmenu->menucallbacks.fnChoice)
			{
...
extern int16_t StartMenu(				// Returns 0 on success.
	...
	if (pmenu)
		...
		// If there is a callback . . .
		if (ms_pmenu->menucallbacks.fnInit)
			{
		...
		ms_sNumMenuItems	= 0;
		while (pmenu->ami[ms_sNumMenuItems].pszText)
			{
			ms_sNumMenuItems++;
			}
		...
		if (ms_pmenu->menuheader.pszFontFile)
			{
		...
		if (ms_pmenu->menuitemsfont.pszFile)
			{
			// Attempt to allocate get font resource . . .
			if (rspGetResource(
				ms_presmgr,
		...
		if (ms_pmenu->menuback.pszFile)
			...
			if (rspGetResource(
				...
				// If paletted . . .
				if (ms_pimBackground->m_pPalette)
					{
		...
		if (ms_pmenu->menuindicator.pszFile)
			{
			...
			if (bAcceptChoice == true)
				...
				if (ms_pmenu)
					...
					if (ms_sCurItem == ITEMINDEX(ms_pmenu->menuautoitems.sCancelItem))
						...
						if (ms_pmenu->menuautoitems.pmenuBack)
							{
