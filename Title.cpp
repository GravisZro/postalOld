...
static bool IsInList(   // Returns true if in list.  false otherwise.
   char const *   pszSearchFor,  // In:  String to search for in pszSearchIn.
   char const *   pszSearchIn)   // In:  Comma delimited list of strings to search in for
                        // pszSearchFor.
   {
   bool  bFound   = false; // Assume not found.

   // Copy to temp so strtok can tokenize leaving full of nullptrs.
   char  szTokenize[512];
   strncpy(szTokenize, pszSearchIn, sizeof(szTokenize) - 1);
   szTokenize[sizeof(szTokenize) - 1] = '\0';

   // Tokenize.
   char const *   pszToken = strtok(szTokenize, TOKEN_DELIMITERS);
   while (pszToken)
      {
...
static int16_t DisplayImage( // Returns nothing.
   char const *   pszImageFile)     // Filename of image (relative path).
   {
