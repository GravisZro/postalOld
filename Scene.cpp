...
CScene::Render3D(
   ...
   ASSERT(ps3Cur->m_psop);
   ASSERT(ps3Cur->m_ptex);
   ASSERT(ps3Cur->m_pmesh);
   ASSERT(ps3Cur->m_ptrans);
   ASSERT(ps3Cur->m_psphere);

   // If there's a parent . . .
   if (ps3Cur->m_psprParent)
      {
...
// ****END TEMP****
   ...
   if (sClipLeft < sDiameter && sClipTop < sDiameter && sClipRight < sDiameter && sClipBottom < sDiameter)
      ...
      if (sClipLeft > 0 || sClipTop > 0 || sClipRight > 0 || sClipBottom > 0)
         {
         ASSERT(m_pipeline.m_pimClipBuf);
         // Use clip image.
