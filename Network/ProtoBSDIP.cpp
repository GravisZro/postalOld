   ...
   if (!ms_bDidStartup)
      ...
      if (WSAStartup(MAKEWORD(1,1), &ms_WSAData) == 0)
         ...
         if (LOBYTE(ms_WSAData.wVersion) == 1 && HIBYTE(ms_WSAData.wVersion) == 1)
            ...
            if (WSASetBlockingHook(RProtocolBSDIP::BlockingHook))
               {
            ...
            if (m_sock != INVALID_SOCKET)
               ...
               if ((sOptionFlags & RSocket::optDontCoalesce) && !sResult)
                  ...
                  if (sType == RSocket::typStream)
                     {
                     int optval = 1;
                     if (setsockopt(m_sock, IPPROTO_TCP, TCP_NODELAY, optval, sizeof(optval)) == SOCKET_ERROR)
                        {
                        sResult = -1;
               ...
               if ((sOptionFlags & RSocket::optDontWaitOnClose) && !sResult)
                  ...
                  if (sType == RSocket::typStream)
                     {
                     int optval = 1;
                     if (setsockopt(m_sock, SOL_SOCKET, SO_DONTLINGER, optval, sizeof(optval)) == SOCKET_ERROR)
                        {
                        sResult = -1;
...
int16_t RProtocolBSDIP::Broadcast(void)           // Returns 0 if successfull, non-zero otherwise
   ...
   if (m_sock != INVALID_SOCKET)
      ...
      if (!m_bListening)
         ...
         int optval = 1;
         ms_funcnum = RSocket::OtherFunc;
         if (setsockopt(m_sock, SOL_SOCKET, SO_BROADCAST, optval, sizeof(optval)) == SOCKET_ERROR)
            {
            sResult = -1;
   ...
   if (m_sock != INVALID_SOCKET)
      ...
      if (!m_bListening)
         ...
         if (m_bConnected)
            ...
            // Send data
            ms_funcnum = RSocket::OtherFunc;
            *plActualBytes = send(m_sock, pBuf, lNumBytes, 0);
  ...
  (void)paddress;
   ...
   if (m_sock != INVALID_SOCKET)
      ...
      if (!m_bListening)
         ...
         // Send data
         ms_funcnum = RSocket::OtherFunc;
         *plActualBytes = sendto(m_sock, pBuf, lNumBytes, 0, (SOCKADDR*)&(paddress->address), paddress->lAddressLen);
         if (*plActualBytes == SOCKET_ERROR)
            {
            *plActualBytes = 0;
            int iErr = WSAGetLastError();
   ...
   if (m_sock != INVALID_SOCKET)
      ...
      if (!m_bListening)
         ...
         if (m_bConnected)
            ...
            // Receive data
            ms_funcnum = RSocket::OtherFunc;
            *plActualBytes = recv(m_sock, pBuf, lMaxBytes, 0);
            if (*plActualBytes == SOCKET_ERROR)
               {
               *plActualBytes = 0;
               int iErr = WSAGetLastError();
  ...
  (void)paddress;
   ...
   if (m_sock != INVALID_SOCKET)
      ...
      if (!m_bListening)
         ...
         // Receive data
         ms_funcnum = RSocket::OtherFunc;
         *plActualBytes = recvfrom(m_sock, pBuf, lMaxBytes, 0, (SOCKADDR*)&(addr.address), (int*)(&addr.lAddressLen));
         if (*plActualBytes != SOCKET_ERROR)
            {
...
int16_t RProtocolBSDIP::GetAddress(               // Returns 0 if successfull, non-zero otherwise
   char const * pszName,                                 // In:  Host's name or dotted address (x.x.x.x)
   ...
   if (ms_bDidStartup)
      ...
      if ((strspn(pszName, "0123456789.") >= strlen(pszName)) && (strchr(pszName, '.')))
         ...
         if (phost)
            ...
            memcpy(&(pip->address.sin_addr), phost->h_addr_list[0], phost->h_length);
            pip->address.sin_port = htons(usPort);
            }
         else
            {
