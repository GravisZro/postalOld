...
static void EnumSaveGamesSlots(Menu *menu)
{
    ...
    for (int i = 0; i < Max; i++)
    {
        ...
        {
            struct tm *tm;
            if ((tm = localtime((const time_t*)&statbuf.st_mtime)))
            {
         ...
         switch (ms_menuaction)
            ...
            case MenuActionSaveGame:
               ...
               if (PickFile("Choose Game Slot", EnumSaveGamesSlots, szFile, sizeof(szFile)) == 0)
                  ...
                  #endif
               sResult = rspSaveBox("SaveGameTitle"_lookup, szFile, szFile, sizeof(szFile), SAVEGAME_EXT);
               if (sResult == 0)
                  {
                  if (Game_SavePlayersGame(szFile, pinfo->Realm()->m_flags.sDifficulty) != SUCCESS)
                     {
                     rspMsgBox(RSP_MB_ICN_EXCLAIM | RSP_MB_BUT_OK, "SaveGameErrorTitle"_lookup,
                      "SaveGameErrorText"_lookup);
                     }
                  }
                    #endif
      ...
      int16_t PrepareRealm(                             // Returns 0 if successfull, non-zero otherwise
         ...
         if (!pinfo->m_bBadRealmMP)
            ...
            if (prealm->DoesFileExist(pinfo->RealmName()))
               ...
               if (prealm->Load(pinfo->RealmName(), false) == 0)
                  {
                  // Startup the realm
                  if (prealm->Startup() == 0)
                     {
               ...
               // to a user of a normal version, so we don't say anything.
                  ...
                  if (!pinfo->IsMP())
                     ...
                     rspMsgBox(
                        ...
                        "AppName"_lookup,
                        "PlayOneRealmOnlyMessage"_lookup);
                     }
...
extern int16_t Play_GetRealmInfo(                 // Returns 0 if successfull, 1 if no such realm, negative on error
   ...
   int16_t sMaxTitleLen /*= nullptr */)           // In:  Max length of returned title, including terminating null
   {
   ASSERT(sRealmNum >= 0);
   ASSERT(pszFile);
