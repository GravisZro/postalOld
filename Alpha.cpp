...
int16_t CAlpha::Blit( // Returns 0 on success.
  ...
  int16_t sSrcX,      // Source coordinate in pimSrc to start effect.  Can be negative.
  int16_t sSrcY,      // Source coordinate in pimSrc to start effect.  Can be negative.
  ...
  RRect*  prc)        // Rectangle to clip Dst to.
{
  ...
  if (m_imMask.m_pData != nullptr || m_imMask.m_pSpecial)
  {
    RImage   imDecompress;
    // Allocate image.
