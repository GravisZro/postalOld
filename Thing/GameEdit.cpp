...
//
//    08/27/97 JMI   Changed file counter messages to be very int16_t (no file
...
struct LINKLINE {
   ...
   bool operator < (const LINKLINE &rhs) const {
      if (this->sX0 > rhs.sX0)
         return false;
...
static int16_t SaveRealm(          // Returns 0 on success.
   ...
   char const * pszRealmName,          // In:  Filename to save as.
...
static int16_t LoadTriggerRegions( // Returns 0 on success.
   char const *   pszRealmName);          // In:  Name of the REALM (*.RLM) file.
...
static void RlmNameToRgnName( // Returns nothing.
   char const *   pszRealmName,     // In:  .RLM name.
...
static void InitFileCounter(        // Returns nothing.
   char const *   pszDescriptionFrmt);       // In:  sprintf format for description of
                                    // operation.
         ...
         case RSP_MB1_PRESSED:
            ...
            if (ms_pthingSel->GetClassID() == CThing::CBouyID)
            {
               ...
               if ((m_pBouyLink0 == nullptr && m_pBouyLink1 == nullptr) ||
                   (m_pBouyLink0 != nullptr && m_pBouyLink1))
               {
         ...
         case CThing::CNavigationNetID:
            ...
            if (prealm->m_asClassNumThings[CThing::CNavigationNetID] > 1)
            {
               ...
               if (ms_pthingSel == prealm->GetCurrentNavNet())
               {
                  ...
                  while (bSearching && pNext->m_powner)
                  {
