...
extern int16_t InputSettingsDlg_InitMenu(   // Returns 0 on success.
   ...
   char const * const * papszInputDescriptions = nullptr;   // Descriptions of input values.
...
//       pgui->m_u32TextShadowColor = GetCurrentMenuBox()->m_u32TextShadowColor;
   ...
   // Add reset.
   pmenu->ami[sInputIndex].pszText  = "RotationSetupMenu_RestoreDefaults"_lookup;
   // Enable item.
   pmenu->ami[sInputIndex].sEnabled = TRUE;
   ...
   // Add reset.
   pmenu->ami[sInputIndex].pszText = "RotationSetupMenu_RestoreDefaultsOld"_lookup;
   // Enable item.
   pmenu->ami[sInputIndex].sEnabled = TRUE;
